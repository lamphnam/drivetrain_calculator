...
\begin{itemize}
\item Cài đặt chi tiết các module cơ khí ngoài Module 1, 3 và 4 (ví dụ: thiết kế trục, ổ lăn) trong chuỗi tính toán của hệ thống dẫn động.
...
\section{Hiện thực các nhóm chức năng demo theo yêu cầu đề bài}

Phần này phân tích mức độ đáp ứng của hệ thống đối với năm nhóm chức năng demo theo yêu cầu đề bài. Với mỗi nhóm, báo cáo trình bày ý nghĩa của yêu cầu, cách tiếp cận hiện thực trong phạm vi dự án, và các artifact có thể kiểm chứng khi demo. Cách tổ chức này giúp giảng viên đối chiếu trực tiếp giữa đề bài và sản phẩm thực tế mà không cần tra cứu nhiều nguồn.

\subsection{Module 1: Tiếp nhận và kiểm soát dữ liệu đầu vào}

Đối với nhóm chức năng tiếp nhận dữ liệu đầu vào, hệ thống hiện thực cơ chế kiểm soát hợp lệ ở tầng DTO trước khi kích hoạt các service tính toán. Cụ thể, request \texttt{Module1CalculationRequest} sử dụng các ràng buộc \texttt{@NotNull} và \texttt{@Positive} cho các trường công suất yêu cầu (\texttt{requiredPowerKw}) và tốc độ đầu ra (\texttt{requiredOutputRpm}). Nhờ đó, các trường hợp dữ liệu thiếu hoặc không thuộc miền giá trị hợp lệ được chặn sớm ở tầng API với mã lỗi HTTP 400 kèm thông báo cụ thể cho từng trường vi phạm, trước khi ảnh hưởng đến chuỗi tính toán cơ khí phía sau.

Cơ chế tương tự được áp dụng nhất quán trên toàn bộ hệ thống: \texttt{Module3CalculationRequest} và \texttt{Module4CalculationRequest} đều kiểm tra \texttt{designCaseId} phải dương, và các tham số override (nếu có) phải nằm trong miền giá trị kỹ thuật hợp lệ. Endpoint chính phục vụ nhóm chức năng này là \texttt{POST /api/v1/module-1/calculate}.

Sau khi dữ liệu đầu vào vượt qua kiểm tra, hệ thống thực hiện toàn bộ pipeline tính toán và trả về kết quả chuẩn hóa bao gồm động cơ được chọn, tỷ số truyền các cấp, và thông số động học trên 5 trục (MOTOR, SHAFT\_1, SHAFT\_2, SHAFT\_3, DRUM\_SHAFT). Khi demo, có thể minh họa bằng cách gọi API với giá trị âm để quan sát phản hồi lỗi, sau đó gọi lại với giá trị hợp lệ để nhận kết quả tính toán đầy đủ.

\subsection{Module 2: Hiện thực lõi tính toán cơ khí}

Yêu cầu đề bài đặt ra rằng hệ thống phải thực hiện các bài toán tính toán theo nội dung giáo trình của khoa Khoa học Ứng dụng. Trong phạm vi dự án, không tồn tại một package hay endpoint riêng biệt mang tên ``Module 2''; thay vào đó, ý nghĩa của yêu cầu này được hiện thực phân tán qua ba calculator engine hoạt động tuần tự trong chuỗi tính toán.

Ở tầng đầu tiên, \texttt{Module1CalculationService} thực hiện tính hiệu suất tổng hệ thống ($\eta = \eta_{kn} \cdot \eta_d \cdot \eta_{brc} \cdot \eta_{brt} \cdot \eta_{ol}^3$), xác định công suất yêu cầu động cơ, phân phối tỷ số truyền tổng thành các cấp ($U_1, U_2, U_3$), và tính thông số động học trên từng trục theo công thức $T = 9{,}55 \times 10^6 \cdot P / n$. Ở tầng thứ hai, \texttt{Module3EngineeringCalculator} thiết kế bộ truyền bánh răng côn thẳng: tính chiều dài côn ngoài $R_e$, chọn module tiêu chuẩn $m_{te}$, xác định số răng $z_1, z_2$ và góc côn $\delta$, kiểm nghiệm ứng suất tiếp xúc $\sigma_H \le [\sigma_H]$ và ứng suất uốn $\sigma_F \le [\sigma_F]$, đồng thời phân tích lực ăn khớp ($F_t, F_r, F_a$). Ở tầng thứ ba, \texttt{Module4EngineeringCalculator} thiết kế bộ truyền bánh răng trụ thẳng: tính khoảng cách trục $a_w$, chọn module $m$ từ mảng tiêu chuẩn [1--12\,mm], xác định số răng, kiểm nghiệm ứng suất, và tính lực vòng cùng lực hướng tâm (lực dọc trục $F_a = 0$ cho răng thẳng).

Điểm đáng lưu ý là tất cả calculator engine đều được thiết kế dưới dạng class thuần túy, không phụ thuộc Spring framework và không truy cập database, nhờ đó có thể unit test độc lập với dữ liệu đầu vào tùy ý. Endpoint \texttt{POST /api/v1/drivetrain/full-flow/calculate} cho phép chạy toàn bộ chuỗi tính toán Module 1 $\rightarrow$ 3 $\rightarrow$ 4 trong một lần gọi duy nhất, và response trả về chứa đầy đủ thông số hình học, kết quả kiểm nghiệm ứng suất (pass/fail), cùng vector lực tác dụng lên từng trục.

\subsection{Module 3: Cơ chế lựa chọn và gợi ý cấu hình kỹ thuật}

Yêu cầu đề bài mong muốn hệ thống có khả năng gợi ý hoặc tự động chọn linh kiện phù hợp từ catalog. Trong phạm vi dự án, chức năng này được hiện thực dưới dạng lựa chọn kỹ thuật xác định (deterministic engineering selection) dựa trên các tiêu chí thiết kế, không phải dựa trên machine learning hay hệ thống khuyến nghị thống kê.

Về mặt triển khai, cơ chế lựa chọn hoạt động ở ba cấp độ. Thứ nhất, \texttt{Module1CalculationService} tự động chọn động cơ phù hợp nhất từ cơ sở dữ liệu 9 động cơ, dựa trên tiêu chí công suất định mức $\ge$ công suất yêu cầu và số vòng quay gần nhất với $n_{sb}$ tính được. Thứ hai, Module 3 cho phép chọn vật liệu bánh răng từ catalog gồm 3 loại thép (C40XH quenched/tempered, 40Cr normalized, C45 normalized), mỗi loại có bộ thông số cơ tính riêng phục vụ tính ứng suất cho phép. Thứ ba, cả Module 3 và Module 4 đều tự động chọn module răng ($m_{te}$ hoặc $m$) từ mảng giá trị tiêu chuẩn [1, 1.25, 1.5, 2, 2.5, 3, 4, 5, 6, 8, 10, 12]\,mm dựa trên khoảng cách trục tính được, đồng thời xác định số răng $z_1$ và $z_2$ sao cho tỷ số truyền thực tế sai lệch không quá 4\% so với yêu cầu.

Các artifact có thể kiểm chứng khi demo gồm: response của Module 1 chứa block \texttt{selectedMotor} với mã động cơ, công suất và rpm đã chọn; response của Module 3 và 4 chứa các trường \texttt{moduleMSelected}, \texttt{teethZ1}, \texttt{teethZ2} thể hiện kết quả lựa chọn tự động từ catalog tiêu chuẩn.

\subsection{Module 4: Nền tảng API phục vụ ứng dụng Web/Mobile}

Yêu cầu đề bài đặt ra rằng hệ thống phải tồn tại dưới dạng ứng dụng Web hoặc Mobile để người dùng tương tác. Trong phạm vi repository hiện tại, dự án tập trung vào việc xây dựng tầng backend REST API hoàn chỉnh, đóng vai trò nền tảng dữ liệu và tính toán cho bất kỳ client nào (web, mobile, hoặc công cụ kiểm thử như Postman).

Về mặt triển khai, hệ thống cung cấp 4 nhóm endpoint (Module 1, Module 3, Module 4, Full-flow) với đầy đủ chức năng calculate và history. Đặc biệt, endpoint \texttt{POST /api/v1/drivetrain/full-flow/calculate} cho phép mobile client thực hiện toàn bộ chuỗi tính toán trong một lần gọi duy nhất, giảm thiểu số lượng round-trip cần thiết trên mạng di động. Bên cạnh đó, tài liệu \texttt{docs/api/module4\_api\_contract.md} mô tả chi tiết cấu trúc request/response kèm TypeScript interfaces và Kotlin data classes, giúp mobile developer tích hợp mà không cần đọc source code backend. Postman collection tại \texttt{postman/drivetrain-calculator.postman\_collection.json} chứa 10 request với test script tự động, có thể dùng trực tiếp để demo tương tác với hệ thống trong buổi trình bày.

Cần lưu ý rằng giao diện Web/Mobile hoàn chỉnh không nằm trong phạm vi repository này. Backend và API đã hoàn thành và sẵn sàng phục vụ client; giao diện người dùng có thể được phát triển tách riêng dựa trên API contract đã cung cấp.

\subsection{Module 5: Đặc trưng CNPM thông qua áp dụng mẫu thiết kế}

Theo yêu cầu đề bài, mỗi nhóm phải hiện thực một tính năng đặc trưng phản ánh hướng chuyên môn của đồ án. Nhóm thực hiện theo hướng Công nghệ phần mềm (CNPM), do đó tính năng đặc trưng được thể hiện qua việc áp dụng các mẫu thiết kế (design patterns) trong kiến trúc backend một cách có chủ đích và có thể kiểm chứng. Nhóm không claim bonus theo hướng AI (machine learning) hoặc HTTT (visualization dashboard).

Ba mẫu thiết kế thuộc danh mục GoF được áp dụng trong hệ thống, mỗi mẫu giải quyết một bài toán kiến trúc cụ thể:

\subsubsection{Facade Pattern (GoF -- Structural)}

Class \texttt{FullFlowCalculationService} đóng vai trò Facade, ẩn đi toàn bộ quy trình điều phối ba module tính toán phía sau một method duy nhất. Controller \texttt{FullFlowCalculationController} chỉ giữ một dependency và gọi một method; bên trong, Facade điều phối tuần tự Module 1 $\rightarrow$ Module 3 $\rightarrow$ Module 4, xử lý truyền dữ liệu giữa các module và tổng hợp kết quả. Cách tiếp cận này tách biệt orchestration logic khỏi HTTP layer, cho phép thêm module mới mà chỉ cần sửa Facade mà không ảnh hưởng đến API contract.

\subsubsection{Strategy Pattern -- Priority Cascade (GoF -- Behavioral)}

Method \texttt{resolveStresses()} trong \texttt{Module4CalculationService} hiện thực cơ chế giải quyết giá trị ứng suất cho phép từ nhiều nguồn theo thứ tự ưu tiên: (1) giá trị do người dùng cung cấp trực tiếp trong request, (2) kế thừa từ kết quả Module 3 nếu tồn tại, (3) giá trị mặc định hệ thống. Enum \texttt{ValueSource} (REQUEST, MODULE3, DEFAULT) ghi nhận chiến lược nào được chọn, và record \texttt{StressResolution} đóng gói cả kết quả lẫn metadata nguồn gốc. Thông tin truy vết này được xuất ra trong trường \texttt{calculationNotes} của response, giúp người dùng hiểu rõ giá trị đến từ đâu.

\subsubsection{Builder Pattern (GoF -- Creational)}

Tất cả entity chính của hệ thống (\texttt{Module4Result}, \texttt{Module3Result}, \texttt{Module1Result}) sử dụng Lombok \texttt{@Builder} để xây dựng object theo từng bước. Với hơn 25 trường trong \texttt{Module4Result}, việc sử dụng builder chain (dòng 61--92 trong \texttt{Module4CalculationService}) loại bỏ hoàn toàn nguy cơ nhầm thứ tự tham số so với constructor truyền thống, đồng thời giúp code xây dựng entity đọc được như documentation. Annotation \texttt{@Builder.Default} được dùng cho các trường có giá trị mặc định, đảm bảo tính nhất quán khi không phải mọi trường đều được set tường minh.

\vspace{0.3cm}
\noindent Báo cáo chi tiết với đầy đủ code evidence cho từng pattern được trình bày trong tài liệu \texttt{docs/cnpm/design\_patterns\_report.md}.

\subsection{Kịch bản minh họa khi demo}

Để trình bày hệ thống trong buổi demo, nhóm đề xuất kịch bản sau nhằm bao quát cả năm nhóm chức năng trong một luồng liên tục:

\begin{enumerate}
\item Khởi động backend bằng lệnh \texttt{./mvnw spring-boot:run} và xác nhận ứng dụng sẵn sàng.
\item Nạp dữ liệu demo (nếu database trống) bằng các script SQL trong thư mục \texttt{database/demo-seed/}.
\item Mở Postman và import collection từ \texttt{postman/drivetrain-calculator.postman\_collection.json}.
\item Gọi endpoint full-flow với bộ tham số mẫu:
...
\end{lstlisting}
\item Trình bày kết quả Module 1: động cơ được chọn tự động, tỷ số truyền các cấp, thông số động học trên 5 trục.
\item Trình bày kết quả Module 3: thông số hình học bánh răng côn, kết quả kiểm nghiệm ứng suất, vector lực trên trục.
\item Trình bày kết quả Module 4: hình học bánh răng trụ, trạng thái pass/fail của ứng suất, lực vòng và hướng tâm.
\item Minh họa cơ chế validation: gọi lại API với \texttt{requiredPowerKw = -1} và quan sát phản hồi lỗi HTTP 400 kèm thông báo chi tiết.
\item Mở source code và chỉ ra ba design patterns (Facade, Strategy, Builder) như tính năng đặc trưng theo hướng CNPM.
\item Gọi \texttt{GET /api/v1/drivetrain/full-flow/history/100} để minh họa khả năng tra cứu lịch sử tính toán.
\end{enumerate}

\subsection{Giới hạn phạm vi và các hướng không claim}

Hệ thống hiện tại tập trung vào backend REST API và calculation engine. Giao diện Web/Mobile được hỗ trợ thông qua API contract và Postman collection, tuy nhiên việc hiện thực giao diện người dùng hoàn chỉnh nằm ngoài phạm vi repository này. Các module cơ khí ngoài Module 1, 3 và 4 (ví dụ: thiết kế trục, ổ lăn) chưa được hiện thực trong backend ở giai đoạn hiện tại.

Về tính năng đặc trưng, nhóm chỉ claim theo hướng CNPM (sử dụng design patterns có bằng chứng cụ thể trong source code). Nhóm không claim bonus theo hướng AI (machine learning) hoặc HTTT (visualization dashboard) vì không có hiện thực tương ứng trong hệ thống.
